...
Use `npx evolink-topaz-video-upscale@latest -y --path {SKILLS_DIR}` only as a fallback when `skills.sh` or the skills registry is unavailable.
...
Publication is owner-controlled. If republishing is approved, use `scripts/publish-npm.sh`; it stages `README.npm.md` as `README.md`, runs `npm pack --dry-run --json --ignore-scripts`, prefers token-backed publish when `NODE_AUTH_TOKEN` is present, and keeps `npm publish --access public --auth-type=web` as the browser-auth fallback without requesting an OTP.
